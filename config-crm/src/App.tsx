...
function Logo({ size = 'lg' }: { size?: 'lg' | 'sm' }) {
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      <div className={`${isLg ? 'w-10 h-10 sm:w-12 sm:h-12' : 'w-8 h-8'} rounded-xl bg-gradient-to-br from-emerald-500 to-green-600 flex items-center justify-center shadow-lg shadow-emerald-500/20`}>
        <Truck className={`${isLg ? 'w-5 h-5 sm:w-6 sm:h-6' : 'w-4 h-4'} text-white`} />
      </div>
      <span className={`${isLg ? 'text-2xl sm:text-3xl' : 'text-lg'} font-black tracking-tight`}>
