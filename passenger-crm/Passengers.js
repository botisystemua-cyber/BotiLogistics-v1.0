...
let routeDateFilter = null;  // null | 'DD.MM.YYYY' — фільтр по 'Дата виїзду' лідів усередині маршруту
...
async function openRoute(idx, forceRefresh) {
    // При перемиканні маршруту скидаємо фільтр дат — у новому маршруті
    // свої дати, старий вибір там просто не знайде жодного ліда.
...
function getFilteredRouteRows(rows) {
    ...
    if (routeDateFilter) {
        filtered = filtered.filter(r => formatTripDate(r['Дата виїзду'] || '') === routeDateFilter);
    }
    return filtered;
}

// Унікальні дати виїзду серед лідів маршруту (для чіпів дат).
// Повертає відсортований (зростання) масив «DD.MM.YYYY».
function getRouteUniqueDates(rows) {
    const set = new Set();
    (rows || []).forEach(r => {
        const d = formatTripDate(r['Дата виїзду'] || '');
        if (d && d !== '—') set.add(d);
    });
    // Сортуємо за реальною датою (DD.MM.YYYY → YYYYMMDD для порівняння)
...
function renderRoutes() {
    ...
    // Чіпи дат виїзду — зверху списку лідів у маршруті. Унікальні дати з
    // rawRows (до інших фільтрів), щоб юзер бачив усі наявні дати навіть
    // коли інший фільтр вже звужує результат.
    ...
    if (uniqueDates.length > 0) {
        ...
        html += '<span class="route-date-chips-label">📅 Дата виїзду:</span>';
        uniqueDates.forEach(d => {
            const active = (routeDateFilter === d);
            const cnt = rawRows.filter(r => formatTripDate(r['Дата виїзду'] || '') === d).length;
...
function renderPaxPopup(tripPax, title, maxSeats) {
    ...
    if (tripPax.length === 0) {
        ...
        tripPax.forEach(p => {
            ...
            const seat = p['Місце в авто'] || '';
            const isFlex = p['Гнучке'] === true || p['Гнучке'] === 'true';
            const flexBadge = isFlex ? '<span title="Вільна розсадка (можна посунути)" style="margin-left:4px;font-size:10px;">🔀</span>' : '';
            const seatInfo = seat ? `<span class="pax-seats" style="background:#eff6ff;color:#1e40af;font-size:10px;">${seat}</span>` : '';
            paxHtml += `<div class="pax-item">
                <div>
                    <div style="font-weight:600;">${name}${flexBadge}</div>
...
function getOccupiedSeats(calId, excludePaxId) {
    ...
    passengers.forEach(p => {
        if (p['CAL_ID'] === calId && p['PAX_ID'] !== excludePaxId) {
            ...
            const isFlex = p['Гнучке'] === true || p['Гнучке'] === 'true';
            seats.forEach(s => { map[s] = { label: label, flexible: isFlex, paxId: p['PAX_ID'] }; });
        }
    });
    return map;
}
...
function findFirstFreeSeat(trip, excludePaxId, extraExcluded) {
    ...
    const occupied = getOccupiedSeats(trip.cal_id, excludePaxId);
...
function buildSeatHtml(seatObj, occupiedMap) {
    ...
    if (isOcc) {
        const occLbl = (typeof isOcc === 'object') ? isOcc.label : isOcc;
        const flexC = (typeof isOcc === 'object' && isOcc.flexible) ? ' sp-flex' : '';
        return `<div class="sp-seat sp-occupied${flexC}"><div class="sp-seat-num">${s.name}</div><div class="sp-occ-name">${occLbl}</div></div>`;
    }
    if (isSel) {
