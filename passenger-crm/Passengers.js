    ...
    if (pcList) {
        if (routes.length === 0) {
            ...
            pcList.innerHTML = routes.map((sheet, idx) => {
                const name = (sheet.sheetName || '').replace('Маршрут_', '');
...
async function assignToRoute(routeIdx) {
    ...
    try {
        ...
        hideLoader();
        if (res.ok) {
            showToast('✅ ' + leadsData.length + ' лід(ів) перенесено в ' + sheetName.replace('Маршрут_', ''));
            clearSelection();
...
async function saveOptimizedToRoute(routeIdx) {
    ...
    try {
        ...
        hideLoader();
        if (res.ok) {
            showToast('✅ ' + leadsData.length + ' лід(ів) перенесено в ' + sheet.sheetName.replace('Маршрут_', '') + ' (оптимізований порядок)');
            clearSelection();
            loadRoutes();
        } else {
            showToast('❌ ' + (res.error || 'Помилка переносу'));
...
function autoOptimizeRoutePrompt(routeIdx, sheetName) {
    ...
    const name = sheetName.replace('Маршрут_', '');
...
// Перезаписати порядок рядків у маршруті після оптимізації
async function reorderRouteAfterOptimize() {
    ...
    const nonPaxRows = ctx.allRows.filter(r => !(r['Тип запису'] || '').includes('Пасажир'));

    // Побудувати оптимізований порядок
    const byId = {};
    for (const r of paxRows) byId[r._resolvedId || r['RTE_ID']] = r;

    const orderedPax = [];
    for (const id of optimizedOrderIds) {
        if (byId[id]) orderedPax.push(byId[id]);
    }
    // Додати тих, хто не потрапив в оптимізацію
    for (const r of paxRows) {
        if (!optimizedOrderIds.includes(r._resolvedId || r['RTE_ID'])) orderedPax.push(r);
    }

    // Видаляємо всіх пасажирів і додаємо заново в правильному порядку
    showLoader('Оновлення порядку в маршруті...');
    try {
        // Видалити всіх пасажирів
        for (const r of paxRows) {
            await apiPost('deleteFromSheet', { sheet: sheetName, id_col: 'RTE_ID', id_val: r['RTE_ID'] });
        }

        // Додати заново в оптимізованому порядку
        const leadsData = orderedPax.map(r => {
            const row = {};
            for (const key of Object.keys(r)) row[key] = r[key];
            return row;
        });
        await apiPost('addToRoute', { sheetName, leads: leadsData });

        // Перезавантажуємо дані маршруту з сервера (forceRefresh)
        if (activeRouteIdx !== null && routes[activeRouteIdx]) {
            routes[activeRouteIdx].rows = null;
            await loadRouteSheetData(activeRouteIdx, true);
        }
        hideLoader();
        renderRoutes();
        showToast('✅ Порядок в маршруті оновлено!');
        window._optimizeRouteContext = null;
    } catch (e) {
        hideLoader();
        showToast('❌ Помилка: ' + e.message);
